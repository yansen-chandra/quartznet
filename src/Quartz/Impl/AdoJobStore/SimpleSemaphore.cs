	...
	public class SimpleSemaphore : ISemaphore
	{
		...
		public virtual void ReleaseLock(string lockName)
		{
			lock (this)
			{
				...
				{
					log.DebugException(string.Format(CultureInfo.InvariantCulture, "Lock '{0}' attempt to retun by: {1} -- but not owner!", lockName, Thread.CurrentThread.Name), new Exception("stack-trace of wrongful returner"));
				}
			}
		}
